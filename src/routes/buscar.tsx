      ...
      <div className="px-5">
        ...
        <div className="mt-3 flex items-center gap-2 overflow-x-auto pb-1">
          ...
          {[4.5, 4.0, 3.5].map((r) => (
            <button
              key={r}
              onClick={() => navigate({ search: (p: z.infer<typeof searchSchema>) => ({ ...p, rating: p.rating === r ? 0 : r }) })}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  value={min || ""}
                  placeholder="$0"
                  onChange={(e) => navigate({ search: (p: z.infer<typeof searchSchema>) => ({ ...p, min: Number(e.target.value) || 0 }) })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => navigate({ search: (p: z.infer<typeof searchSchema>) => ({ ...p, max: Number(e.target.value) || 0 }) })}
                  className="w-full h-10 px-3 rounded-xl bg-muted/50 border border-border outline-none focus:border-foreground/30 text-sm"
                />
              </div>
            </div>
